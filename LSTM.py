...
for p in range(len(stocks)): #len(stocks)
    ...
    for r in range(len(yy_test)): # We train dynamically de data
        ...
        model = Sequential()
        model.add(LSTM(32, return_sequences=True, input_shape=(X_train.shape[1],X_train.shape[2]))) #the second Dim of input_shape is the number of features
...
with open('df_results_summary_LSTM_strat_28.pickle', 'wb') as handle:
...
with open('df_loss_per_stock_LSTM_strat_28.pickle', 'wb') as handle:
